  ...
  return (
    <div className='h-fit w-[100%] pb-10 p-2 flex flex-col items-center '>
      <div className='w-[100%] flex justify-center items-center gap-5 md:justify-end'>
        ...
        <Cart cartCount={cartCount} toggleBetweenCartList={()=>toggleBetweenCartList()}/>
      </div>
      {showCartList ? (
            choosedItem.length > 0 ? (
              <CartList choosedItem={choosedItem} />
            ) : (
      ...
      <div className='items-arrange' hidden={showCartList}>
        {
          itemsData.map((item)=>(
            <Card item={item} key={item.id} addToCart={()=>addToCart(item)} removeFromCart={()=>removeFromCart(item.id)}/>
          ))
        }
      </div>
